...
def signup():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('auth.verify_email', user_id=u.id))
...
def forgot_password():
    ...
    if form.validate_on_submit():
        ...
        if u and u.email_verified:
            ...
            flash('OTP sent to your email.', 'info')
        return redirect(url_for('auth.forgot_password_verify', user_id=u.id))
...
def forgot_password_verify(user_id):
    ...
    if form.validate_on_submit():
        if (u.email_otp == form.otp.data
            ...
            u.email_otp_expires = None
            db.session.commit()
            return redirect(url_for('auth.forgot_password_reset', user_id=user_id))
        flash('Invalid or expired OTP.', 'danger')
